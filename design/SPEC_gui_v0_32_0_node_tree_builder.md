...
- **Impl review round 1 (YELLOW → folded, 2026-06-10; persisted at `design/agent-reports/gui-v0_32_0-node-tree-impl-r1-review.md`):** I1 the surplus-flag-RENDERS kittest cell added (the visible half of the funds-safety inversion). M3 strip wire-absence token. M5 the tracing flake serialized (rebuild_interest_cache + bounded retries; 10/10) — its FOLLOWUP resolved. M2/M4 filed as FOLLOWUPs. **Two SPEC deviations ACCEPTED at review (errata):** (1) §0's parse contract reads descriptor-FIRST — the success envelope carries BOTH `descriptor` and `diagnostics: []`, so the literal diagnostics-first wording would mis-route success; the binding intent is stdout-parse-keyed, never exit-code-keyed. (2) §3's Edit-as-tree uses a PURPOSE-BUILT argv (archetype + declared params + `--emit-spec` only) — `--emit-spec` clap-conflicts with `--json`/`--format`, so assemble_argv reuse would break the feature.
