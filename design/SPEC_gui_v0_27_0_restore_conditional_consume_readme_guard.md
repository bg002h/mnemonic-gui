...
- `src/schema/mnemonic.rs`: (a) bump the module-doc (`:1` `mnemonic-toolkit-v0.46.0` → `v0.46.2`) + `pinned_version` (`:3687` `"mnemonic 0.46.0"` → `"mnemonic 0.46.2"`); (b) **(R0 I2) rewrite the stale comment on the `restore` `SubcommandSchema` (`:3454-3457`)** — its "The toolkit gui-schema still emits `conditional_rules: []` for restore (hand-encoded allowlist has no restore arm) … (not drift-gated, like repair/inspect)" is now FALSE; rewrite to: toolkit v0.46.2 now projects the restore rule (`cmd/gui_schema.rs` `restore_conditional_rules()` arm), so `--from required-unless-md1` IS drift-gated by `gui_schema_conditional_drift` (`("restore", 1)` in `SUBCOMMAND_FLOORS` this cycle); the `conditional: Some(crate::form::conditional::restore)` wiring already matches (no logic change). **Anti-blind-`sed`:** the `:2746` provenance comment (`toolkit v0.46.0: scan a file of candidate passphrases…`) legitimately documents when `--passphrase-candidates-file` was added (v0.46.0) and MUST stay — bump the version strings + rewrite `:3454-3457` ONLY; do NOT touch `:2746`.
- **(R0 I1) `src/form/conditional.rs:926-934` doc-comment on `restore()` is now STALE — REWRITE (no logic change; the fn body `:935-941` already matches).** Four clauses become false once the pin reaches v0.46.2: "hand-encoded allowlist … with no `restore` arm", "restore emits `conditional_rules: []`", "not covered by `gui_schema_conditional_drift`", and "Promotion … tracked as a toolkit FOLLOWUP". Rewrite to: toolkit v0.46.2 NOW projects this rule (`cmd/gui_schema.rs` `restore_conditional_rules()` + the `"restore"` dispatch arm) as `not(flag_present "--md1") → {--from, required}`, so it IS drift-gated by `gui_schema_conditional_drift` with `("restore", 1)` in `SUBCOMMAND_FLOORS` (this cycle) — and DROP the "tracked as a FOLLOWUP" sentence (resolved this cycle). The shape was authored to match the toolkit projection (it does).
- **(R0 M1) `tests/conditional_visibility.rs:1076-1078` comment is the same stale class** ("hand-encoded allowlist with no restore arm … not drift-gated") — one-line touch-up alongside I1 (the test stays GREEN; it calls `run_conditional` directly, not via the drift path). Re-point it to "now toolkit-projected + drift-gated (toolkit v0.46.2)".
