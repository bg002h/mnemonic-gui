...
M = [
    ...
    ("NI8: channel-lookalike refusal off", 'if looks_like_channel(v):', 'if False:', True, False),
    ("NI8: normalization weakened (no whitespace strip)", 'return v.strip().casefold()', 'return v.casefold()', False, False),
    ("NI8: normalization weakened (no case-fold)", 'return v.strip().casefold()', 'return v.strip()', False, False),
    ("NI9: trailing CR/LF refusal off", 'if POLICY["refuse_trailing_cr_lf"] and v[-1:] in ("\\r", "\\n"):', 'if False:', True, False),
    ("NI9: the ONE strip-one rule becomes strip-all",
     '"strip-one-trailing-newline": lambda raw: raw[:-2] if raw.endswith("\\r\\n") else raw[:-1] if raw.endswith("\\n") else raw,',
     '"strip-one-trailing-newline": lambda raw: raw.rstrip("\\r\\n"),', False, False),
