...
- **Severity:** secret-handling — logged, non-gating (operator ruling 2026-08-27). **Status:** RESOLVED on branch `gui-followups` (unreleased): `secrets::text_value_is_secret_node_token` (node in `SECRET_NODE_TYPES_ARGV`, value not empty/`-`/`@env:`) drives the argv mask, `should_confirm_run`, `redact_for_persistence` and a `.password` + reveal-eye Text widget; `tests/restore_from_secret_node.rs` (12 tests, each site mutation-checked, public `xpub=` negative controls). **Owning phase:** next GUI secret-handling cycle.
...
- **Status:** ✓ **RESOLVED (this PR).** The GUI already surfaced the verdict SAFELY (never a false "verified OK") — `runner.rs` returns non-zero exits as `Ok(RunResult)` and renders exit code + stdout/stderr verbatim, so a partial dead card always showed `exit 4` + the VERIFY-ME note. This PR adds a DISTINCT visual state: `render_exit_badge()` (`src/app_window.rs`) gains a `Some(4)` arm rendering an amber "exit: 4 — VERIFY-ME (confirm the result out-of-band before trusting)" badge — matching the toolkit's project-wide exit-4 VERIFY-ME class (SPEC §4: bundle/seed mismatch, bounded-distance repair candidate, or the v0.88.0 pathless PARTIAL decode). Amber (220,165,0) distinguishes it from success (default label) and exit-5 Repair-Applied (green); the specific `result: partial`/`mismatch` verdict + note render verbatim in the panes below (no fragile wire-shape parsing). **Severity:** LOW-MEDIUM (UX; the underlying safety was already correct).
- **Tier:** cross-repo (GUI consumer side). **Companion:** `mnemonic-toolkit/design/FOLLOWUPS.md` → `verify-bundle-json-partial-result` (filed OPEN at `mnemonic-toolkit-v0.88.0`; flip to RESOLVED when this GUI PR merges).

### `canonical-origin-sh-wpkh-toolkit-mirror-divergence` — GUI `classify_descriptor_canonicity()` regex table will misclassify `sh(wpkh)` as non-canonical once the toolkit pin bumps past v0.87.0

- **Surfaced:** 2026-07-11, toolkit Track-B re-pin cycle (`mnemonic-toolkit-v0.87.0`, `toolkit-repin-sh-wpkh-canonical-flip`). md-codec 0.41.0's F-A1 flips `canonical_origin(sh(wpkh(@N)))` `None`→`Some(m/49'/0'/0')`, so the toolkit now classifies `sh(wpkh(...))` as **canonical** (`mnemonic gui-schema --classify-descriptor 'sh(wpkh(...))'` → `canonical`; was `non-canonical`). The GUI's `src/form/conditional.rs:58-101` `classify_descriptor_canonicity()` reimplements md-codec's canonical table as **5 wrapper-form regexes** with NO `sh(wpkh)` arm, so it still returns `NonCanonical` for `sh(wpkh)`. Its lagging agreement gate `tests/canonicity_drift.rs` shells out to `MNEMONIC_BIN gui-schema --classify-descriptor` and WILL catch the disagreement (`disagreements`/`wrong_verdict`) — but only once the pinned toolkit binary reports the new verdict.
- **Status:** ✓ **NO DRIFT YET / NO GUI CODE CHANGE THIS CYCLE.** The GUI's toolkit pin is FROZEN at `mnemonic-toolkit-v0.75.0` (`pinned-upstream.toml:22`), far behind v0.87.0, so `canonicity_drift.rs` still runs against the pre-flip binary and stays green. **Action at the NEXT GUI toolkit-pin bump (→ ≥ v0.87.0):** add a 6th regex arm for `sh(wpkh(@N))` to `classify_descriptor_canonicity()` (mirroring the wrapper-form prefix style of the other 5), update the `Canonicity::Canonical` doc-comment "5 wrapper shapes" → "6", and add/adjust the `canonicity_drift.rs` fixture for `sh(wpkh)` (expect `Canonical`). This is a same-PR-as-the-pin-bump obligation (paired-PR discipline); it does not block the toolkit release. **Severity:** LOW (a UI classification banner; the toolkit is the authoritative gate, and the drift gate REDs at the pin bump if the regex isn't updated first).
- **Tier:** cross-repo (mirror-divergence, GUI consumer side). **Companion:** `mnemonic-toolkit/design/FOLLOWUPS.md` → `canonical-origin-sh-wpkh-toolkit-mirror-divergence` (RESOLVED `mnemonic-toolkit-v0.87.0`).

### `sibling-gui-schema-v5-default-value-emission` — md/ms/mk-cli should emit v5-style `default_value` in `gui-schema` so a two-sided md/ms/mk defaults gate becomes possible

- **Surfaced:** 2026-07-11, FOLLOWUP burndown GUI batch (S2 / eval #6 extension). `tests/schema_mirror_defaults_drift.rs`'s defaults/choices drift gate covers `mnemonic` (v5 `gui-schema`) two-sidedly (`default_value` + `choices`), but `md`/`ms`/`mk`'s `gui-schema` is still **version 1** — it emits ZERO `default_value` fields for any flag (R0-verified live: `md`/`ms`/`mk gui-schema` at the pinned tags `descriptor-mnemonic-md-cli-v0.11.0` / `ms-cli-v0.13.0` / `mk-cli-v0.11.0` OMIT the `default_value` key entirely — 0/35 flags carry it on `md`, likewise `ms`/`mk`; serde `#[serde(default)]` on the consumer side resolves the absent key to `None`). This batch extended the gate to md/ms/mk **CHOICES-only** (`json_flag_choices`, 16 dropdown flags across the three: md 5, ms 8, mk 3) plus a SELF-ARMING one-sided defaults guard ("IF the JSON ever carries a non-null `default_value`, it must equal the hand mirror") — vacuously green today, arming automatically once a sibling starts emitting v5. A true two-sided defaults gate (mirror-present ⟹ JSON-present, catching a silent mirror omission the way `mnemonic`'s gate does) is infeasible until the siblings emit v5-style `default_value`.
- **Fix (if pursued):** md-cli/ms-cli/mk-cli bump their `gui-schema` emitter to v5 parity with the toolkit's (per-flag `default_value` populated from clap-derive defaults), then bump this repo's `pinned-upstream.toml` pins + re-point the S2 one-sided guard to a full two-sided gate (mirroring `mnemonic_defaults_and_choices_match_pinned_gui_schema`). Needs sibling releases + a GUI pin bump — a future cross-repo cycle.
- **7 mirror-default backfills that ride this** (the schema-mirror `default_value` fields that would populate once v5 lands, per `--help`'s `[default: ...]`): `md encode --network`, `md verify --network`, `md address --network`, `md address --chain`, `md address --index`, `md address --count`, `ms encode --language`.
- **Status:** OPEN. **Tier:** `cross-repo` (md-codec + mnemonic-secret + mnemonic-key `gui-schema` producer side; mnemonic-gui consumer side re-points the gate once the pins bump). **Companion:** `descriptor-mnemonic/design/FOLLOWUPS.md`, `mnemonic-secret/design/FOLLOWUPS.md`, `mnemonic-key/design/FOLLOWUPS.md` → `sibling-gui-schema-v5-default-value-emission` (each sibling's `gui-schema` v1→v5 emitter bump).

### `gui-dropdown-none-opts0-materialization-audit` — broad audit of `Dropdown+default_value:None` flags that materialize `opts[0]` into argv

- **Surfaced:** 2026-07-10, F6 (`mnemonic-gui-v0.58.0`, eval-F5+F6). A rendered form writes `Dropdown(opts[0])` into `state.values` for a `default_value:None` dropdown and `is_at_default` can't suppress it (`widget.rs:220-228` → `flag_defaults.rs:30-32`; only `Dropdown("")` escapes via the `emit_one` `!v.is_empty()` guard). A census (F6 whole-diff review) found **33** such flags emitting their `opts[0]` untouched. **Most are benign** (`opts[0]` == the toolkit's own default). F6 fixed ONLY the PROVEN funds bug (`address-of-xpub` `--address-type`/`--network`, where the toolkit INFERS so `opts[0]` overrides inference). **This entry tracks the rest:** classify each of the ~31 remaining as "benign (== toolkit default)" vs "overrides inference / a real bug", and apply the `""`-prepend `(none)` sentinel to the latter. **Top-priority target: `verify-bundle --template`** (materialized `bip44` vs an md1-carried type → a false verify-FAIL; fail-SAFE direction — the keyless-template md1 path short-circuits before template mode at toolkit `verify_bundle.rs:379-400` — so a loud false-negative, not fail-deadly). Also folds the still-open `--network` dropdowns from [[network-dropdown-default-forces-explicit-mainnet]] (`convert`/`export-wallet`). **De-migration note (F6 M-2):** the F6 I-3 persistence migration is unconditional — a post-v0.58.0 user who DELIBERATELY selects `p2pkh`/`mainnet` on address-of-xpub loses it on restart; a version-stamped one-shot migration is the future refinement. **Severity:** LOW→MEDIUM per flag (the `verify-bundle` case is a real false-verdict). **Tier:** `mnemonic-gui`. **Companion:** `mnemonic-toolkit/design/FOLLOWUPS.md` → `gui-dropdown-none-opts0-materialization-audit`.
...
- **Where (refreshed @ v0.48.0):** three surfaces in `src/form/tree_form.rs` — `posix_pipeline_command` (`:124`, the slug names only this one), `spec_json_pretty` (`:104`, the "Copy spec JSON" button), `spec_stdin_bytes` (`:94`, the live `--spec -` stdin); all serialize `tree_model::to_spec_json` (`src/form/tree_model.rs:438`). Call sites `src/main.rs:929` / `:934` / `:950`. A real fix MUST cover all three (the slug under-scopes to the first).
...
- **Why it is currently safe (verified R0-r2; re-verified Wave-2):** build-descriptor's policy tree is over XPUBS (watch-only) — the 17 `NODE_KIND_SPECS` kinds (`src/schema/nodes.rs`: pk/pkh/multi/sortedmulti/older/after/sha256/hash256/hash160/ripemd160/and_v/or_d/or_i/or_b/andor/thresh/wrap) are DISJOINT from the 9 `SECRET_NODE_TYPES_ARGV` tokens. `TreeNode.kind` is always a `NODE_KIND_SPECS` kind; `to_spec_json` emits only `key`/`keys` (watch-only xpubs), `k`/`n` (uints), `hex` (public digests), `w` (wrap) — no field can structurally carry a secret-class value. So there is no secret to leak TODAY. This is a defense-in-depth gap, not a live leak.
- **Residual coherence (Wave-2 G2):** an xprv mis-pasted into `hex`/`w` is now persist-BLANKED on-disk (`tree-xprv-heuristic-only-covers-key-fields`, v0.48.1) but still flows verbatim through `to_spec_json` into this transient clipboard/stdin leg — same fat-finger threat, asymmetric coverage (durable on-disk leg closed, transient clipboard leg open). Tracked here, DEFERred (transient, no durable persistence; bounded).
- **Trip-wire (auto-re-arm):** the compile-time `const _: () = assert!(secret_slice_eq(...))` at `src/secrets.rs:80`/`:91` FAILS THE BUILD if a toolkit pin bump adds a secret node-type, auto-re-arming this slug; backstopped by `tests/secret_taxonomy_pin.rs` + `tests/spec_nodes_mirror.rs`.
- **Fix when needed:** if a future build-descriptor node ever carries a secret-class value, add a JSON-redaction pass chokepointed at `to_spec_json` (or a shared redact-pass) covering ALL THREE surfaces above, keyed on `secrets::node_type_is_argv_secret` (the wider ARGV superset). Re-verify the watch-only assumption at that time.
- **Status:** open (deferred / conditional — no live leak; auto-re-armed by the compile-time drift assert).
...
- **Where (reconciled @ v0.48.0):** `src/form/tree_model.rs::blank_non_extended_public_keys` (`:714`) — the persistence-redaction walk; the original slug cited `blank_xprv_keys`, a fn RENAMED in the v0.34.0 allowlist inversion (audit I6). It sweeps `key` + `keys[i]` only (SPEC §1.3, deliberately: hashlock `hex` digests must survive). The `hex` (hashlock digest) and `w` (wrapper string) free-text widgets accept arbitrary paste, so a mis-pasted xprv in either persisted to `state.json` verbatim.
- **What:** extend the redaction walk to `hex` + `w` via the NARROW `is_xprv_like` (`:675`) DENY-list (NOT the `is_extended_public_like` allowlist key/keys use — that would blank every legit 64-hex digest / wrap string = data-loss). Free belt-and-suspenders, since neither field is ever legitimately xprv-shaped (the heuristic can't false-positive on a 64-hex digest or a wrap-char string: `prv` at byte offset 1..4 — p/r/v ∉ hex alphabet, structurally guaranteed). Keep-hex-digests posture preserved; only xprv-MATCHING content blanks.
- **Status:** **resolved** `mnemonic-gui-v0.48.1` (2026-06-22, Wave-2 secret-hygiene G2). `blank_non_extended_public_keys` gains a deny-list arm blanking `node.hex`/`node.w` iff `is_xprv_like(..)` (asymmetric to the key/keys allowlist — correct: key/keys have no legit non-key shape, hex/w do). On-disk persist walk ONLY (the in-RAM `zeroize_keys` twin is left unchanged per SPEC §1.3 — the durable on-disk leg is the higher-value exposure). The `redacted_for_persistence` doc-comment amended. Test: `tests/persist_redaction_v0_34_0.rs::t6_tree_persist_blanks_xprv_in_hex_and_w` (xprv→blanks in hex/w incl. recursion; legit 64-hex digest + "sv" wrap SURVIVE — the survive cells guard against the rejected fail-closed shape). PATCH (private-fn redaction widening; no signature/wire/pub-field/Drop change). R0-GREEN (0C/0I). **Residual (DEFERred, `tree-mode-posix-pipeline-spec-json-unmasked`):** an xprv into `hex`/`w` is still emitted verbatim by `to_spec_json` into the Copy-spec-JSON / posix-pipeline / `--spec -` transient clipboard leg — durable on-disk leg closed, transient clipboard leg tracked separately (no live secret-class node today; auto-re-armed by the compile-time drift assert).
- **Tier:** RESOLVED (was: GUI-local belt-and-suspenders; the redaction walk + its unit cells were the touchpoints).
...
- **Status:** **resolved** `mnemonic-gui-v0.39.0` (2026-06-11; flipped 2026-06-22 in the Wave-2 G2 shipping commit per followup-status-discipline — the core ask was satisfied at v0.39.0 but the slug was never flipped). The cited premise is now FALSE: the modal masks each secret value token. Shipped (verified @ v0.48.0): `SECRET_MASK = "••••"` (`src/form/invocation.rs:137`), `assemble_argv_with_secret_mask` (`:152`, parallel `(Vec<String>, Vec<bool>)` correct-by-construction across all secret-value sources), `render_copy_command_masked` (`:524`); the run-confirm modal substitutes `SECRET_MASK` per masked token at `src/main.rs:1098-1104` (the real argv still spawns on Run); `PendingConfirm.mask` + `impl Zeroize + Drop` (`src/runner.rs`, cycle-15 Lane G). CHANGELOG line 81/85. The FOLLOWUP's option (b) (per-token check at the modal site) was effectively taken. Coverage: `tests/secret_mask_preview_v0_39_0.rs` (11 tests), `tests/secrets.rs` run-confirm gate, `tests/run_holder_zeroize.rs`, `tests/widget_secret_mask_cycle15g.rs`. The cross-repo manual companion (`gui-run-confirm-modal-secret-redaction-manual-companion`) ships in lockstep as a paired toolkit manual-prose PR (Wave-2 G1-B).
- **Tier:** RESOLVED (was: `v0.4-cross-repo`).
- **Companion:** `mnemonic-toolkit/design/FOLLOWUPS.md` `gui-run-confirm-modal-secret-redaction-manual-companion`; `mnemonic-toolkit/docs/manual-gui/src/10-foundations/14-secret-handling.md` Defense-2 prose patch. Closure required: (i) GUI source patch implementing redaction — DONE v0.39.0; (ii) manual prose patch undoing the v1.0 honest-broken framing and restoring the `••••` claim — Wave-2 G1-B paired toolkit PR; (iii) `pinned-upstream.toml` bump in that manual to the GUI tag shipping the fix (`mnemonic-gui-v0.48.1`) — Wave-2 G1-B.
...
- **Class:** identical to the already-tracked `gui-secret-buffer-allocator-residue` / `gui-os-snapshot-secret-occlusion` caveats (referenced in `PASTE_WARN_MODAL_TEXT`). The values/slots/positionals/secret_widgets the exit sweep already covers carry the same undo-ring caveat; M9 brought `state.tree` to parity with them. This facet specifically names the build-descriptor tree `key`/`keys[i]` `TextEdit`s, whose model `String` is zeroized on exit (M9) but whose egui undo ring is not.
- **Disposition (Wave-2 G3, 2026-06-22):** **won't-fix-cleanly / accepted-residue (documented caveat)** — BLOCKED ON UPSTREAM egui 0.31.1. A true RAM-scrub is not cleanly achievable: egui owns the undo `String`s inside `Arc<Mutex<Undoer<(CCursorRange, String)>>>` (egui-0.31.1 `src/widgets/text_edit/state.rs`), fed the REAL buffer every frame independent of `.password(..)` (so display-masking does not keep the secret out of the ring). The only public lever, `TextEditState::clear_undoer()`, REPLACES the `Undoer` with `default()` and DROPS the old `VecDeque`s — drop frees but does NOT overwrite (egui has zero Zeroize, repo-wide). So even the "clean" path leaves the secret in freed-but-unscrubbed heap = the same allocator-residue class already ratified-accepted → zero net hygiene gain. Reaching `clear_undoer` would also need the exact per-frame-derived widget `Id`s (not recorded in the model) — brittle, silently no-ops on an egui Id-derivation shift; and eframe `on_exit(&mut self)` has no `&egui::Context`. A genuine elimination needs a zeroizing/scrubbing undo buffer (or opt-out) upstream — neither exists in 0.31.1. Shipping a `clear_undoer()` pass + RESOLVED flip would OVER-CLAIM (drop ≠ scrub) and risk a silent regression on the next egui bump. Consistent with the sibling rulings `gui-secret-buffer-allocator-residue` + `gui-os-snapshot-secret-occlusion`. Already shipped (do NOT redo): `TreeNode::zeroize_keys` (M9, v0.46.0) scrubs the model Strings; `.password(is_xprv_like(..))` masks the display (cycle-15g, v0.47.0).
- **Status:** documented-caveat (won't-fix-cleanly; upstream-egui-blocked).
...
## `gui-inspect-json-template-field` — `mnemonic inspect --json` wire-shape change (template + schema_version 2)

- **Surfaced:** 2026-06-28, toolkit v0.75.0 (`mnemonic-inspect-md1-template-output`) post-impl whole-diff review (M-gui).
- **What:** toolkit v0.75.0 added a `template` field to the `mnemonic inspect --json` md1 body and bumped the shared `INSPECT_SCHEMA_VERSION "1"→"2"` (so ms1/mk1 `--json` envelopes also report `"2"`; their bodies are unchanged). This is a **runtime `--json` wire-shape change**, which the `schema_mirror` gate does NOT cover (it gates clap flag-NAMES only, not `--json` output shape) — so it does not auto-fail, but a strict consumer of `inspect --json` could break on the new field / version.
- **Action:** if `mnemonic-gui` consumes `mnemonic inspect --json` output, verify it tolerates the additive `template` field and `schema_version:"2"`; update the consumer + any pinned wire-snapshot accordingly. No clap-flag/schema-mirror change is needed (the inspect flag surface is unchanged). Paired-PR discipline (the un-gated `--json` wire-shape class).
- **Status:** `open` (catalog; verify-on-next-GUI-pin-bump). **Tier:** `cross-repo` (un-gated wire-shape). **Companion:** `mnemonic-toolkit/design/FOLLOWUPS.md::mnemonic-inspect-md1-template-output` (RESOLVED v0.75.0) + `descriptor-mnemonic/design/FOLLOWUPS.md::md-codec-descriptor-to-template-renderer`.

## `gui-prefilled-default-text-appends-on-type` — editable Text/Path flags pre-fill their schema default, so typing without clearing concatenates

- **Surfaced:** 2026-06-29, the P5 UI-harness coverage sweep (`gui-automated-ui-functionality-harness`). The sweep's 6 I1 round-trip "failures" were all this: a Text/Path flag with a non-empty schema `default_value` (`compare-cost/--feerate`=`1.0`, `export-wallet/--output`+`restore/--output`=`-`, `import-wallet/--select-descriptor`=`all`, `nostr/--timestamp`+`ms derive/--account`=`0`) is pre-filled as EDITABLE text (`src/form/widget.rs:214-223` seeds the default into `state.values`; `render_row:520` binds it editably). A user who types into the field without first clearing gets `1.0` + `5` = `1.05`, or `--output -` → `-/path`.
- **Why MINOR (not a wiring bug):** the render→store→argv wiring is CORRECT — a cleared-then-typed value persists faithfully (proven: the harness seeds `Text("")` and all 342 round-trips go green). It is a UX papercut (visible in the field + the masked copy-command preview, recoverable), not a funds/functional defect. The P5 sweep found **0 functional bugs**; this is the one usability item it surfaced.
- **Action:** for flags whose `default_value` is a real intended value, the pre-fill is arguably correct (clearing is normal text-field behavior); for flags where the "default" is really a sentinel/hint (notably `--output -` = stdout sentinel), prefer egui `hint_text`/placeholder so typing replaces rather than appends. Decide per-flag. Possibly a select-all-on-focus affordance for defaulted fields.
- **Status:** ✓ **RESOLVED (2026-07-01, `fix/hint-text-defaults`).** Text/Path schema defaults are now DISPLAY-ONLY: `default_flag_value_for_flag` empty-seeds Text/Path (`src/form/flag_defaults.rs` — the single resolver, so widget seed + emit `seeded_fixture` + `.gui` value column moved atomically) and the widget renders the default as an egui `hint_text` GHOST (`src/form/widget.rs` `render_row` Text/Path arms; precedent `slot_editor.rs`) — typing REPLACES, never appends; an empty field means "the CLI applies its own default". Zero argv bytes changed (D33 already omitted at-default values; pinned by `tests/hint_text_defaults.rs` argv invariants + the AccessKit `value == ""` anti-tautology anchor). Stale pre-fix autosaves are normalized by a load-time, Text/Path-scoped, fail-open migration (`persistence.rs::normalize_loaded_form_values` — drops a persisted value EQUAL to its schema default; `Text("")`/`Path("")` survive). The `.gui` emit depicts `<hint:DEFAULT>` (grammar sibling of `<empty>`/`<unset>`); the 6-PNG snapshot corpus re-pinned (exactly the 6-flag blast radius above). The sweep's `Text("")`/`Path("")` re-seed workaround is REMOVED — every Text/Path I1 round-trip is now a permanent append-regression tripwire. The 6-flag census above is the full papercut surface (verified: zero conditional/`src/` consumers read any of the 6). Design: `mnemonic-toolkit/docs/manual-gui/design/SPEC_gui_hint_text_defaults.md` (R0 ×2 → GREEN) + `agent-reports/hint-text-spec-r0-round-{1,2}.md`. The select-all-on-focus alternative was rejected (spec §9 N5) — ghosting also communicates "empty = CLI default". Sibling item surfaced by the same recon filed below: `gui-number-set-affordance-ignores-schema-default`. **Tier:** `ux`. **Companion:** `gui-automated-ui-functionality-harness` (the harness that found it).

## `gui-number-set-affordance-ignores-schema-default` — the Number `Set` affordance seeds the range `min`, not the schema `default_value`

- **Surfaced:** 2026-07-01, the hint-text-defaults recon (`SPEC_gui_hint_text_defaults.md` §9 N4; example verified by its R0 round-1 review).
- **Where:** `src/form/widget.rs:379-381` — `seeded_value_for` maps `FlagKind::Number { min, .. } => FlagValue::Number(*min)`, ignoring `flag.default_value` entirely (it takes a `&FlagKind`, so it cannot even see the schema default).
- **What:** clicking `Set` on a Number flag whose schema declares a non-`min` default seeds the range MIN, not the default. Canonical example: `xpub-search --gap-limit` (`src/schema/mnemonic.rs:3133-3143` — `min: 0`, `default_value: Some("20")`): `Set` yields `0`, and because `0 ≠ 20` the D33 `is_at_default` suppression does NOT fire — the argv EMITS `--gap-limit 0`, silently overriding the CLI default of 20. This is **argv-affecting** (unlike the display-only hint-text papercut), which is exactly why it was scoped OUT of the hint-text UX cycle rather than folded in.
- **Action:** make the `Set` seed flag-aware (a `seeded_value_for_flag(&FlagSchema)` sibling preferring a parseable `default_value` over `min`, falling back to `min` / kind-only behavior), or ghost the default on the Unset row; either way add an argv-level regression (post-fix, `Set`-then-untouched on `--gap-limit` seeds 20 → `is_at_default` suppresses → argv omits). Doc shadow: the GUI manual's `50-md/53-encode.md` "spin-box pre-filled with `5`" prose was stale before this cycle (Number renders `Unset` + `Set`) and is corrected in the hint-text cycle's Phase M as a pre-existing-inaccuracy fix.
- **Status:** `open`. **Tier:** `ux` (argv-affecting — higher urgency than display-only papercuts).
