...
- **Companion (2026-06-10):** the toolkit-side classification fix SHIPPED — **toolkit v0.53.1** classifies `--phrase`, `--phrase-stdin`, AND `--ms1-stdin` as `secret: true` (`mnemonic-toolkit/design/FOLLOWUPS.md::vacuous-secret-flag-gate` + `::gui-secret-mirror-phrase-ms1-stdin`). At the next toolkit pin bump this repo must: (1) add the 3 tokens to `src/secrets.rs`'s token-for-token mirror (the secret-drift gate will RED otherwise), (2) flip **9** hand-coded `secret:` sites in `src/schema/mnemonic.rs` (3 flags × 3 xpub-search subcommands — `--phrase` :2280/:2442/:2712, `--phrase-stdin` :2291/:2453/:2723, `--ms1-stdin` :2312/:2474/:2744 at 036776b; the toolkit modes carrying `--phrase` are path-of-xpub / passphrase-of-xpub / account-of-descriptor — NOT address-of-xpub, which has `--xpub`). The `ms repair --ms1` sibling entry below is NOT covered by the toolkit change (ms-cli surface).
- **Status:** open (toolkit half done; GUI flips pending pin bump).
