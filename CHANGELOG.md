...
- **ms 0.19.1, not 0.19.0:** 0.19.0 took an `--allow-argv-secret`-admitted value for a stdin read, so the GUI's `ms verify` with an ms1 card and `--phrase` failed with "cannot read both ms1 and --phrase from stdin". 0.19.1 fixes it (verify, derive, combine); its `gui-schema` is byte-identical to 0.19.0, so no mirror change. A real-binary test pins it.
