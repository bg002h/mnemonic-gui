...
**SemVer-MINOR — pinned to, and exercised against, today's CLIs (F-679): `mnemonic` 0.104.0, `md` 0.20.3, `ms` 0.19.0, `mk` 0.13.0.** Toolkit dependency tag `mnemonic-toolkit-v0.97.0 → v0.104.0` (the load-bearing pin; `Cargo.toml` now also carries the toolkit's `[patch.crates-io]` rust-miniscript rev, without which md-codec 0.47.0 does not compile); `pinned-upstream.toml` md `v0.11.0 → v0.20.3`, ms `v0.16.0 → v0.19.0`, mk and toolkit as above.

- **Secret material on argv — the Run path opts in, the Copy path does not.** `mnemonic` and `ms` now REFUSE a secret on the command line unless `--allow-argv-secret` is given, so every secret-bearing GUI run (a `--slot @0.phrase=`, a `--passphrase`, an ms1 positional, `restore --from ms1=`…) was refused. `--allow-argv-secret` is mirrored on all 32 `mnemonic` and 8 `ms` subcommands for schema parity but is **GUI-managed**: no widget, never emitted from form state. `Run` inserts it right after the subcommand when the argv carries secret material (the confirm modal shows it); `Copy command` never does, so a pasted command still meets the CLI's refusal in a shell whose history would keep the secret. See FOLLOWUPS `argv-secret-via-private-channels` for the stdin/`--in` end state.
- **New flags surfaced:** `export-wallet --allow` (repeatable; only `sigless-branch` is enforced there) and `--count` (default 20) with `--format bitcoin-core-addresses`; `restore --recalibrate-threads`; `md` `--in` on inspect/encode/decode/verify/bytecode/repair, `encode --out`, `--experimental` on encode/verify/address, `--path` on verify/address, `address --from-mk1 / --from-mk1-file / --seat`; `ms` `--in` on all 8 material verbs and `--out` on encode/repair/split; `ms derive --template` gains `bip48-p2tr`, `bg002h-tr`, `bg002h-wsh`. Conditionals: `ms encode --in` joins the phrase/hex one-of; `md encode --in` counts as the template input; `md address --from-mk1` and `--template` exclude each other.
- **Defaults and values that moved:** `--group-size` defaults to `0` (unbroken) on `bundle` / `convert` / `ms-shares split|combine`; `--separator` offers `space` only on `mnemonic`, `md` and `ms` (hyphen/comma are refused there now; `mk` 0.13.0 still takes all three). `ms repair --ms1` and the md1-string positionals are no longer marked required (`--in` is the alternative).
- **`--` before positionals, always.** Every command the GUI builds (Run, Copy, Preview, confirm modal — one argv) now ends its options with `--` when it carries positionals. Without it `md address --from-mk1 <mk1>… <md1>` let the multi-value `--from-mk1` swallow the policy card (md exit 1). Every mirrored subcommand that takes positionals was measured to accept `--` with identical output.
- **ms input-source exclusivity:** `inspect`/`decode`/`verify`/`derive`/`repair`/`combine` get conditionals so `--in` cannot be sent alongside the positional (or `--ms1`, `--phrase`, `--hex`) that clap refuses it with; the first filled source wins and later flag sources grey out. `md address` marks the policy-card positional Required when only `--from-mk1` is given.
- **Deliberately NOT surfaced yet:** md `compose`, `shape-key`, `descriptor`, `decompose`; ms `hashlock` (FOLLOWUPS `md-ms-new-subcommands-unsurfaced`).
...
- **Secret-field reveal (👁) toggle** — secret fields were hardcoded `.password(true)` with no way to see what you typed (a seed you can't verify is a footgun; and the tutorial couldn't show the reader what to enter). A reveal affordance is added beside every on-load-masked secret field (sites #1 `secret_widget.rs`, #2 `slot_editor.rs` secret arm, #3 `widget.rs` composite; build-descriptor tree-key sites #4/#5 deferred). **Hygiene model (load-bearing):** hold-to-reveal (pointer) primary + a bounded latch (keyboard/AccessKit/capture) fallback, both feeding one per-frame `.password(!reveal)`; a single Context-transient revealed-field `Id` (never a `FormState` field → the never-persist net is structurally unaffected); auto-hide on Run / field-blur / window-focus-loss / tab-or-subcommand switch; no timeout in v1; a pointer tap does **not** latch. **Display-ONLY:** the run-confirm modal, argv echo / copy-command, paste-warn, persistence, and exit-sweep all stay masked/redacted **unconditionally**, regardless of reveal state. +12 hygiene TDD cells; the faithfulness gate models the 👁 `Role::Button` non-vacuously.
- **restore `--template` `(none)` unset** — the restore form materialized a single-sig `--template=bip44` default that the `mnemonic` CLI **rejects in multisig `--md1` mode** (exit 2), with no escape. A `(none)` sentinel is now APPENDED (new `RESTORE_TEMPLATES` const; `opts[0]` stays `bip44`, `default_value` stays `None`; the shared `TEMPLATES` and its bundle/verify-bundle/convert consumers untouched) so the user can clear the template and complete the md1 restore. Render-scoped (F1 mirror); the md1-mode mutex projection is deferred as a paired cross-repo follow-up.
- **Combined tutorial re-drive** — the 50-shot corpus is regenerated once: the 6 md1-restore steps now drive the clean `(none)` path (dropping the multisig-template workaround); the 4 rule-derived secret steps (`Drive::secret_value().is_some()`: j1-01, j2-02, j2-03, j2-07) **reveal their public demo phrase** so the reader sees exactly what to type; every other secret shot gains the 👁 chrome but stays masked. The `assert_no_plaintext` filled-form checkpoint is allowlist-bounded (only the 3 public demo phrases, revealed); the confirm-modal and populated-pane checkpoints stay unconditionally strict. Transcripts byte-identical; corpus byte-deterministic (27.1 MiB ≤ 32).
- **Gallery:** the 61-form structural corpus gains the 👁 on 32 secret-bearing figures (28 `.gui`-emit forms — the eye is emitted as an ASCII ` [reveal]` marker — ∪ 4 composite-default-secret forms, visual only). New FOLLOWUP `tutorial-assert-no-plaintext-value-equality-skip` filed (a latent, inert test-oracle hardening).
- **Gates:** full suite green; clippy both configs; I3 never-persist / `schema_mirror` / `conditional_drift` all unchanged.
...
- **Combined `SHA256SUMS` release asset** (`.github/workflows/build.yml` `release` job). One `SHA256SUMS` over every release asset — both `*.tar.gz` (musl + gnu + darwin) and `*.zip` (windows) — emitted after `download-artifacts` and before the softprops `github-release` step, written under `artifacts/` so the existing `files: artifacts/**/*` glob uploads it. The step `-type f`-filters (matching `*.tar.gz` or `*.zip`) and `cd`s into each asset's directory so the checksum lines carry the published basenames (`actions/download-artifact@v5` nests each artifact under `artifacts/<artifact-name>/` and the artifact name ends in `.tar.gz` / `.zip`), so `sha256sum -c SHA256SUMS` passes against a flat downloaded asset. The shipped guarantee is *static + checksummed*, not bit-for-bit reproducible.
...
- **`gen-man` SubcommandSchema added to all four schema files** (`src/schema/{mnemonic,md,ms,mk}.rs`). One required directory-path flag `--out`. The toolkit's `mnemonic gen-man` ALSO carries the global `--no-auto-repair` flag (clap-derive propagates it to every `mnemonic` subcommand; `gui-schema` emits both), so the `mnemonic` `GEN_MAN_FLAGS` includes `NO_AUTO_REPAIR_FLAG` — the sibling CLIs have no global flags, so their `gen-man` exposes `--out` only. `--out` is modeled `FlagKind::Path` for mnemonic/md/mk and `FlagKind::Text` for ms (mirroring each CLI's `gui-schema` `kind` emission; the gate compares flag-NAME sets only). No conditional rules.
- **Pin bump:** `mnemonic-toolkit-v0.70.0 → v0.73.0`, `descriptor-mnemonic-md-cli-v0.7.0 → v0.11.0`, `ms-cli-v0.8.0 → v0.13.0`, `mk-cli-v0.9.0 → v0.11.0` — across `pinned-upstream.toml`, the Cargo.toml toolkit dep tag + Cargo.lock, the README install lines, and the four schema `pinned_version` display strings. Cargo.lock re-resolves the toolkit's transitive `ms-codec 0.6.0 → 0.7.0` and adds the man-page deps `clap_mangen` + `roff` (codex32 dropped from the toolkit's dep set; recompile-only — the GUI consumes neither's changed surface).
- **Gates:** `schema_mirror` (4 CLIs, 21 subtests) GREEN against the v0.73.0 / md-v0.11.0 / ms-v0.13.0 / mk-v0.11.0 binaries (`MNEMONIC_BIN`/`MD_BIN`/`MS_BIN`/`MK_BIN`); `pin_coherence`, `readme_pin_coherence`, `gui_schema_conditional_drift`, `wire_shape_snapshot`, full `cargo test` (547 tests) GREEN; `cargo clippy --all-targets` clean. No `cargo fmt` (GUI has no fmt CI gate).

## mnemonic-gui [0.49.0] — 2026-06-22

**SemVer-MINOR — Wave-3 W3-1 keystone: per-consumer `--json` WIRE-SHAPE golden-snapshot regression tests (+ a tiny W3-3 mk-help prose mirror).** Closes the option-(b) residual of FOLLOWUP `schema-mirror-flag-name-vs-wire-shape-conceptual-clarification`: the `schema_mirror` gate enforces clap flag-NAME parity but NOT the runtime `--json` wire-shape of the subcommands the GUI consumes. The new `tests/wire_shape_snapshot.rs` (8 live-capture cells) pins the wire SHAPE — top-level + nested key SETS, the match-vs-no_match conditional-key drift class, and stable scalar values — for `xpub-search` (path/account/passphrase modes) + `import-wallet` (bsms multisig + coldcard single-sig), keyed to the `mnemonic-toolkit-v0.70.0` toolkit pin. `export-wallet` is DESCOPED (it has NO `--json` envelope — its wire-shape is per-`--format` file output; tracked as a follow-on). New test surface + prose → MINOR. No toolkit pin change.

- **`tests/wire_shape_snapshot.rs` (NEW).** 8 cells capture each `--json` envelope FROM the v0.70.0 binary (NOT hand-written; NOT copied from the retired stale v0.27.0 vendored fixtures, which encoded the OLD shape — e.g. `path/template/account: null` on a `no_match`, since corrected to key OMISSION). A full-recursive key-path extractor pins import-wallet envelope shape so a key add/remove ANYWHERE fails; the xpub-search cells pin top-level + matched-cosigner key sets plus the no_match key-omission drift class and the `roundtrip.status` key. CI `schema-mirror.yml` `cargo-test-full-suite` installs the toolkit via `cargo install --tag mnemonic-toolkit-v0.70.0` and runs with `MNEMONIC_BIN=mnemonic`, so the cells RUN against that exact binary; a dev laptop with no binary SKIPS (skip-when-absent, mirroring `schema_mirror.rs`). Maintenance contract: every future toolkit-pin bump MUST re-verify these 8 goldens in lockstep — the LEADING drift gate the slug wants.
- **Retired `tests/cli_envelope_smoke.rs` + its stale v0.27.0 vendored fixtures** (`tests/fixtures/v0_27_0_envelopes/*`, `tests/fixtures/wallet_import/envelope_v0_27_0.json`). The loose presence-checks over stale fixtures never ran the binary and never asserted full shape — superseded by the live-capture module (grep-confirmed sole consumer before delete).
- **Vendored `tests/fixtures/wallet_import/bsms-2line-multi-2of3.txt`** (from `mnemonic-toolkit-v0.70.0`) for the bsms-multisig import cell; SOURCE.md provenance row added.
- **W3-3 mk-help prose mirror (`src/schema/mk.rs`):** the `mk vectors --pretty` help string + adjacent comment were corrected from "Silently ignored when --out is set" to "Also honored when --out is set (each per-fixture file is pretty-printed)" — mk-cli source actually honors `--pretty` under `--out`. GUI `help:` prose is free-form and ungated (`schema_mirror` compares flag-NAME sets only), so this is non-drifting; the mk-cli source reword + manual edits are SEPARATE repo cycles (deferred). No flag added/removed/renamed.
- **Gates:** `schema_mirror` (flag-NAME parity), `readme_pin_coherence` (README self-tag bumped to `mnemonic-gui-v0.49.0` in lockstep with Cargo.toml), full `cargo test --workspace` (the 8 new cells run against the cargo-installed v0.70.0 binary), `cargo clippy --all-targets -- -D warnings` — all GREEN. No `cargo fmt` (GUI has no fmt CI gate). R0-GREEN (0C/0I).

## mnemonic-gui [0.48.1] — 2026-06-22

**SemVer-PATCH — Wave-2 secret-hygiene G2: extend the on-disk persist redaction to the tree `hex`/`w` free-text fields.** The persistence-redaction walk `blank_non_extended_public_keys` (`src/form/tree_model.rs`) swept `key`/`keys[i]` only (positive `is_extended_public_like` allowlist, v0.34.0 audit I6) — the `hex` (hashlock digest) and `w` (wrapper) free-text widgets accept arbitrary paste, so a private key fat-fingered into either persisted to `~/.config/mnemonic-gui/state.json` verbatim. Pure internal redaction-coverage widening; no clap-flag / dropdown / `schema_mirror` surface, no wire-shape change, no `pub`-struct/Drop change → PATCH. No toolkit pin change.

- **`blank_non_extended_public_keys` gains a NARROW `is_xprv_like` DENY-list arm** for `node.hex` + `node.w` (`src/form/tree_model.rs`): an xprv-SHAPED mis-paste in either is blanked; legitimate 64-hex hashlock digests and wrap strings (`"sv"` etc.) SURVIVE. Asymmetric to the key/keys ALLOWLIST on purpose — key/keys have no legitimate non-key shape, `hex`/`w` do, so a fail-closed allowlist would destroy valid data. The survival is structurally guaranteed (a 64-hex digest can never carry `prv` at bytes 1..4 — p/r/v ∉ the hex alphabet). On-disk persist walk ONLY; the in-RAM `zeroize_keys` twin is unchanged (the durable on-disk leg is the higher-value exposure). `redacted_for_persistence` doc-comment amended.
- **Closes FOLLOWUP:** `tree-xprv-heuristic-only-covers-key-fields` (G2). Also flips `gui-run-confirm-modal-secret-redaction` → resolved (G1-A; the modal-redaction core ask shipped at v0.39.0 and was never flipped — followup-status-discipline), dispositions `gui-tree-key-egui-undo-ring-residue` → documented-caveat (G3; upstream-egui-blocked — `clear_undoer` drops-but-doesn't-scrub = no net gain), and refreshes the deferred `tree-mode-posix-pipeline-spec-json-unmasked` citations (G4; no live leak — secret node-types disjoint from build-descriptor node-kinds, auto-re-armed by the compile-time drift assert). Test: `tests/persist_redaction_v0_34_0.rs` (T6 — xprv→blanks in `hex`/`w` incl. recursion; legit digest + wrap SURVIVE). R0-GREEN (0C/0I). Paired cross-repo: the toolkit `docs/manual-gui` prose revert + `pinned-upstream.toml` bump to `mnemonic-gui-v0.48.1` (Wave-2 G1-B) ships in lockstep.

## mnemonic-gui [0.48.0] — 2026-06-22

**SemVer-MINOR — schema-mirror lockstep catch-up for the toolkit `v0.60.0 → v0.70.0` pin bump (own-account subset-search).** The GUI pin had fallen 10 toolkit versions behind (the v0.60→v0.69.1 constellation bug-hunt burndown never re-pinned the GUI). A 4-agent recon (`cycle-prep-recon-gui-v0.70.0-catchup.md`), each verified against the live v0.70.0 binary + toolkit source + GUI consuming code, established the COMPLETE accumulated GUI-affecting delta: **exactly 3 new `mnemonic` clap flag-NAMES + one refuse→search behavior flip**. The intervening 9 releases (mostly secret-memory-hygiene + funds-safety bug-fixes) added no other gated CLI surface — dropdown VALUE enums are byte-identical, and the only two `--json` wire-VALUE corrections (M7 `bundle` threshold, M1 `import-wallet` account, both v0.66.0) land on subcommands the GUI renders as raw text (it deserializes only `build-descriptor --json`), so no consumer-code change is owed.

- **`restore` gains 1 flag** in `RESTORE_FLAGS` (`src/schema/mnemonic.rs`): `--search-cosigner-subset` (`FlagKind::Boolean`, non-secret) — opt-in bounded over-supplied-cosigner search (default OFF; cosigners matched exactly).
- **`verify-bundle` gains 2 flags** in `VERIFY_BUNDLE_FLAGS`: `--own-account-max` (`FlagKind::Number`, mirrors `restore --own-account-max`) + `--search-cosigner-subset` (`Boolean`) — the verify mirror, threaded into the same shared `complete_multisig_template` engine.
- **`restore --own-account-max` help + comment rewritten refuse→subset-search.** v0.70.0 flipped this flag from "reserved/refused this cycle" (the v0.44.0-era state) to an active own-anchored k-permutation subset-search. The GUI help string previously read "NOT SUPPORTED YET — refused"; it is now an accurate description of the live search behavior + the `--account` mutex. GUI `help` strings are free-form and ungated, but a flatly-wrong (funds-misleading) string is a real user-visible defect.
- **`--own-account-max ⊕ --account` mutex NOT modeled GUI-side.** The toolkit carries the clap `conflicts_with`, but does NOT project it into `gui-schema conditional_rules` (restore stays 1 rule, verify-bundle 10 — pinned in `tests/gui_schema_conditional_drift.rs`). Adding a GUI conditional rule would BREAK the drift-gate count; the mutex stays a runtime-only concern (documented in the schema comments).
- **Pin bump:** `mnemonic-toolkit-v0.60.0` → `v0.70.0` (Cargo.toml dep tag + Cargo.lock + `pinned-upstream.toml` `[mnemonic].tag` + README install line + schema module-doc header). Cargo.lock also re-resolves the toolkit's transitive `md-codec` 0.37.0 → 0.39.0 and `ms-codec` 0.4.4 → 0.6.0 (recompile-only; GUI consumes neither's changed surface — full suite green).
- **Gates:** `schema_mirror` (4 CLIs, 21 subtests) GREEN against the v0.70.0 binary (`MNEMONIC_BIN`); `pin_coherence`, `readme_pin_coherence`, `archetype_schema_mirror`, `gui_schema_conditional_drift`, `xpub_search_schema_mirror`, `schema_mirror_secret_drift`, `canonicity_drift` green; full suite green. No `cargo fmt` (GUI has no fmt CI gate). Toolkit-only flags → no sibling-codec lockstep.
- **Concurrency note:** authored as v0.47.0 but **retargeted to v0.48.0 and rebased** atop the concurrently-shipped cycle-15 Lane G **v0.47.0** (secret-residue zeroize, PR #15) which took the v0.47.0 slot mid-cycle. That release is GUI-internal secret hygiene, orthogonal to the schema mirror / toolkit pin — no interaction; the catch-up substance is unchanged. Full suite re-run GREEN against the rebased base.

## mnemonic-gui [0.47.0] — 2026-06-21

**SemVer-MINOR — secret-residue zeroize + on-screen masking (cycle-15 Lane G).** Four GUI-local secret-hygiene gaps in surfaces NEITHER the M9 tree-model sweep nor the on-disk redactor reaches: two app-level run holders that sit OUTSIDE `FormState` (so the on-exit `zeroize_form_state` sweep never visits them) and two secret-input widgets that rendered cleartext on screen. GUI-only; no toolkit pin change (`mnemonic-toolkit-v0.60.0` unchanged). No clap-flag / dropdown-value change → no `schema_mirror` impact. No behavior change beyond on-screen masking + RAM scrubbing (input still accepted; secrets still flow to the same form-state/run paths).

- **Run holders now zeroize-on-drop (P1).** `src/runner.rs`: `RunResult` (the captured assembled argv + stdout/stderr) gains `impl Zeroize + Drop` (whole-holder scrub — argv/mask/stdout/stderr; `exit_code` left, non-secret). `PendingConfirm` (the real cleartext argv + tree `--spec -` stdin held while the run-confirm modal is open) is promoted from a bare tuple to a `Zeroize + Drop` struct in the `runner` lib module. The run-confirm modal consume site (`main.rs`) is rewritten E0509-safe (a `Drop` type cannot be field-destructured): it binds the whole `pending` struct, reads its fields by-ref, and passes owned clones on the Run path; the cloned transient drops scrubbed on every frame. `on_exit` scrubs both app-level holders in its post-save sweep block via the new pure `secrets::scrub_app_run_holders` seam (they sit outside `FormState`, so the `form_state` sweep never reached them). In-RAM only; neither holder is serialized.
- **Secret-input widgets now render `.password`-masked (P2).** `src/form/widget.rs`: the `NodeValueComposite` value field (`convert --from phrase=… / minikey=…` etc.) masks gated on a single hoisted `secrets::node_type_is_argv_secret(node)` boolean that also drives the existing paste-warn condition (no double-eval, no render-mask/paste-warn split-brain; the `ui.add(TextEdit::singleline(..).password(..))` form preserves the paste-warn `Response`). `src/form/tree_form.rs`: the build-descriptor tree `key` / `keys[i]` fields mask gated on `tree_model::is_xprv_like(..)` — the same predicate the amber `xprv_hint` keys off, so a mis-pasted xprv-shaped private key masks while watch-only xpubs stay readable.
- **Closes FOLLOWUPs:** `gui-last-run-result-argv-stdout-not-zeroized`, `gui-pending-confirm-argv-not-zeroized`, `gui-composite-secret-value-rendered-cleartext-onscreen`, `gui-tree-key-field-rendered-cleartext-onscreen`. Tests: `tests/run_holder_zeroize.rs` (holder scrub + exit-sweep seam), `tests/widget_secret_mask_cycle15g.rs` (`Role::PasswordInput` masking + paste-warn co-fire + source split-brain pin). R0-gated per-phase + whole-diff review (`mnemonic-toolkit/design/agent-reports/cycle15g-*`).
...
- **`restore` gains 6 flags** in `RESTORE_FLAGS` (`src/schema/mnemonic.rs`): `--own-account-max` (`FlagKind::Number`, reserved/refused this cycle — the subset-search engine is deferred, FOLLOWUP `template-multisig-own-account-range-subset-search` — but it IS a clap flag so it is mirrored), `--search-address` (`Text`), `--search-addr-min` (`Number`, default 0), `--search-addr-max` (`Number`, default 20), `--search-chain` (**value-enum dropdown**, new `SEARCH_CHAINS` const = `["receive", "change", "both"]`, `default_value: Some("receive")`), `--accept-search-time` (`Text`). All non-secret.
- **`verify-bundle` gains 7 flags** in `VERIFY_BUNDLE_FLAGS`: `--from` (`Text`, **non-secret — mirrors `restore --from`'s classification**; the seed source supports `@env:VAR` / `-` stdin for secret values), `--cosigner` (`Text`, repeating, watch-only/non-secret), `--search-address`, `--search-addr-min` (default 0), `--search-addr-max` (default 20), `--search-chain` (`SEARCH_CHAINS` dropdown, default `receive`), `--accept-search-time` — the verify/recompose mirror of the same-named `restore` flags.
- **`restore --account` widened in help text** to the `#28` phase-2 comma-list (`Vec<u32>`, one own account per multisig slot, e.g. `0,1,2,3`). The toolkit's `gui-schema` still reports `kind: number` (clap value-shape is not on the wire), so the GUI keeps `FlagKind::Number` (the Number widget does not model multiplicity) and documents the comma-list in the tooltip only. No gate impact (`schema_mirror` gates flag NAMES).
- **Paired-PR discipline:** `schema_mirror` gates flag NAMES only — the new `--search-chain` dropdown VALUE list (`receive`/`change`/`both`) is mirrored for the paired-PR convention (measured clean vs the v0.60.0 `gui-schema`), same precedent as `--md1-form`/`--separator`/`--archetype`.
- **Pin bump:** `mnemonic-toolkit-v0.59.0` → `v0.60.0` (Cargo.toml dep tag + Cargo.lock rev `2654ba33` + `pinned-upstream.toml` `[mnemonic].tag` + README install line + schema module-doc header). The GUI `Cargo.lock` delta is exactly the two `mnemonic-gui` / `mnemonic-toolkit` version+source lines (the toolkit's transitive `md-codec` 0.37.0 / `mk-codec` 0.4.0 resolve unchanged in the GUI lock).
- **Gates:** `schema_mirror` (4 CLIs) GREEN against the v0.60.0 binary (`MNEMONIC_BIN`) + the unchanged md/ms/mk pins; `archetype_schema_mirror`, `build_descriptor_schema`, `gui_schema_conditional_drift`, `xpub_search_schema_mirror`, `schema_mirror_secret_drift`, `readme_pin_coherence` green; full suite green.
