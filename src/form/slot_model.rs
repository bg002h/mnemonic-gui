//! Egui-free slot data model (extracted from `slot_editor.rs` in the
//! P1 `gui`-feature split — SPEC §3 general invariant). Holds the
//! `--slot @N.<subkey>=<value>` repeating-grammar data types + their pure
//! helpers so the non-gated form model (`schema::FormState.slots`,
//! `persistence`, `secrets`) and the headless `gui-render` emit-mode build
//! WITHOUT egui. The egui-coupled `render(&mut egui::Ui, …)` widget stays
//! in the gated `slot_editor.rs`, which re-exports these types.
//!
