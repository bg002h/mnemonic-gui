//! The real application shell, lifted out of the binary (P0 spike of the
//! `gui_example_tutorial` cycle — SPEC §3.1(a), recon A §5).
//!
//! PURE RELOCATION of `MnemonicGuiApp` (struct, impls, `render_exit_badge`,
//! `spawn_and_capture`, `AUTOSAVE_INTERVAL`) from `src/main.rs` into this
//! `#[cfg(feature = "gui")]` library module so integration tests can drive
//! the REAL window (panels, subcommand combo, output pane) under
//! `egui_kittest`. Three seams, zero behavior change:
//!
//! 1. **Constructor split** — [`MnemonicGuiApp::new_headless`] is the pure
//!    field-init half (injectable [`AppState`]; no OS side-effects). The
//!    production [`MnemonicGuiApp::new`] is now a thin wrapper: the three
//!    `cc`-dependent OS effects (window-capture protection, keepalive
//!    thread, signal handlers) + `AppState::detect_all()` — in today's
//!    execution order — then `new_headless`.
//! 2. **`ui()` extraction** — [`MnemonicGuiApp::ui`] is the former
//!    `update()` body VERBATIM (it was already `eframe::Frame`-free, the
//!    `_frame` param unused); `impl eframe::App::update` reduces to
//!    `self.ui(ctx)`.
//! 3. **Field visibility** — the handful of fields the tutorial harness
//!    reads/seeds are `pub`; the rest stay private. Visibility is not
//!    behavior; the shipped binary is byte-identical in behavior.

use eframe::egui;

use crate::app::{AppState, CliTab};
use crate::form::invocation::{
    assemble_argv_with_secret_mask, render_copy_command, render_copy_command_masked, ShellFlavor,
};
use crate::form::slot_editor::{SlotState, SlotSubkey};
use crate::form::widget;
use crate::help::url as help_url;
use crate::path_detect::Detected;
use crate::persistence::{self, PersistedState};
use crate::runner::{self, PendingConfirm};
use crate::schema::{self, FlagValue, FormState};
use crate::secrets;
...
impl MnemonicGuiApp {
    /// Production constructor — the three `cc`-dependent OS side-effects
    /// (window-capture protection, keepalive thread, signal handlers) +
    /// `AppState::detect_all()`, in the pre-extraction execution order,
    /// then the pure [`Self::new_headless`] field init.
    pub fn new(
        ...
        // session, including the secret-bearing paste-warn modal.
        {
            ...
            if let Ok(handle) = cc.window_handle() {
                crate::platform::apply_window_capture_protection(handle);
            } else {
                tracing::warn!(
        ...
        let app_state = AppState::detect_all();
        Self::new_headless(app_state, loaded, state_path)
    }

    /// Headless constructor — PURE field init only (the pre-extraction
    /// `new()` body minus the three OS side-effects and minus
    /// `detect_all()`, which the caller injects). This is the harness
    /// entry point (SPEC §3.1(b)): tests pass a fixed [`AppState`] (no
    /// `$PATH`-dependent tab grey-out) and `state_path = None` (no
    /// persistence I/O).
    pub fn new_headless(
        app_state: AppState,
        loaded: Option<PersistedState>,
        ...
        if !form_state.contains_key("mnemonic:bundle") {
            form_state.insert(
                ...
                .with_slots(SlotState {
                    rows: vec![crate::form::slot_editor::SlotRow {
                        index: 0,
...
impl MnemonicGuiApp {
    /// The whole-window per-frame render — the former `eframe::App::update`
    /// body VERBATIM (verified `eframe::Frame`-free pre-extraction; the
    /// `_frame` param was unused). `update()` delegates here; the tutorial
    /// harness drives this directly via
    /// `Harness::builder().build_state(|ctx, app| app.ui(ctx), app)`.
    pub fn ui(&mut self, ctx: &egui::Context) {
                ...
                for tab in CliTab::ALL {
                    ...
                    if !available {
                        resp.clone().on_hover_text(
                            crate::app::missing_binary_tooltip(*tab),
                        );
                    }
                    if resp.clicked() {
        ...
        egui::CentralPanel::default().show(ctx, |ui| {
            ...
            let vis = sub
                ...
                .map(|f| f(state))
                .unwrap_or_default();
            let visibility_of = |name: &str| -> crate::schema::Visibility {
                vis.iter()
                    .find(|(k, _)| *k == name)
                    .map(|(_, v)| v.clone())
                    .unwrap_or(crate::schema::Visibility::Visible)
            };
            ...
            egui::ScrollArea::vertical().show(ui, |ui| {
                ...
                if sub.name == "build-descriptor" {
                    crate::form::tree_form::render_mode_selector(ui, state);
                }
                ...
                let archetype_spec = if sub.name == "build-descriptor" && !tree_mode {
                    crate::form::archetype_form::active_archetype(state)
                } else {
                    None
                };
                ...
                for flag in sub.flags {
                    ...
                    if tree_mode
                        && crate::form::tree_form::suppressed_in_tree_mode(
                            flag.name,
                        )
                    {
                        continue;
                    ...
                    if archetype_spec.is_some()
                        && crate::form::archetype_form::suppressed_in_archetype_mode(
                            flag.name,
                        )
                    {
                    ...
                    if matches!(v, crate::schema::Visibility::Hidden) {
                        continue;
                    }
                    ...
                    let disabled_options: Vec<String> = vis
                        ...
                        .filter_map(|(_, v)| match v {
                            crate::schema::Visibility::DisableOptions {
                                values,
                            } => Some(values.clone()),
                            _ => None,
                        })
                    ...
                    ui.add_enabled_ui(
                        !matches!(v, crate::schema::Visibility::Disabled),
                        |ui| {
                    ...
                    if flag.name == "--archetype" {
                        if let Some(spec) = archetype_spec {
                            crate::form::archetype_form::render(
                                ui,
                                active_tab,
                                &active_sub_name,
                                spec,
                            ...
                            crate::form::tree_form::render_edit_as_tree(
                                ui, state, "mnemonic",
                            );
                        }
                    }
                ...
                if sub.allows_slots {
                    ...
                    let path_hint_string = match account_opt {
                        Some(account)
                            if !descriptor.is_empty()
                                && crate::form::conditional::classify_descriptor_canonicity(
                                    &descriptor,
                                ) == crate::form::conditional::Canonicity::NonCanonical =>
                        {
                            let coin =
                                crate::form::conditional::coin_type_for_network(
                                    &network,
                                );
                    ...
                    if let Some(banner) = account_opt.and_then(|account| {
                        crate::form::conditional::descriptor_non_canonical_default_path_notice(
                            &descriptor,
                            &network,
                            account,
                        )
                    ...
                    if let Some(warning) =
                        crate::form::conditional::template_slot_count_warning(
                            template,
                            slot_count,
                        )
                    {
                ...
                for (i, pos) in sub.positional_args.iter().enumerate() {
                    if pos.secret {
                        ...
                        let rows = state
                            ...
                            .entry(key)
                            .or_insert_with(|| {
                                vec![crate::form::secret_widget::SecretLineEdit::new()]
                            });
                        let label = format!(
                            "{} {}{}",
                            pos.name,
                        ...
                        if pos.repeating && ui.button(format!("+ add {}", pos.name)).clicked() {
                            rows.push(
                                crate::form::secret_widget::SecretLineEdit::new(),
                            );
                        }
                        continue;
                    }
                ...
                if tree_mode {
                    crate::form::tree_form::render(ui, state, "mnemonic");
                }
            });
            ...
            if template_changed {
                if let Some(new_template) = &current_template {
                    for (name, default_value) in
                        crate::form::conditional::template_defaults_for(new_template)
                    {
            ...
            let spec_stdin = if tree_mode {
                crate::form::tree_form::spec_stdin_bytes(state)
            } else {
                None
            };
            let spec_copy = if tree_mode {
                crate::form::tree_form::spec_json_pretty(state)
            } else {
                None
            };
            if tree_mode {
        ...
        if ctx.data_mut(|d| {
            d.remove_temp::<bool>(crate::form::secret_widget::paste_warn_id())
                .unwrap_or(false)
        }) {
